...
create table "allowed_statuses" ("uuid" SERIAL NOT NULL PRIMARY KEY,"task" VARCHAR(254) NOT NULL,"status" VARCHAR(254) NOT NULL);
...
alter table "workflows" add constraint "fk_owner" foreign key("task") references "allowed_statuses"("task") on update NO ACTION on delete NO ACTION;

# --- !Downs

alter table "tasks" drop constraint "fk_owner";
alter table "tasks" drop constraint "fk_status";
alter table "workflows" drop constraint "fk_owner";
